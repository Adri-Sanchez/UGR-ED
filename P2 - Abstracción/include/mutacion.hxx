...
mutacion::mutacion(const std::string & str){
	...
	if(str.find("CAF=", inicio)!=std::string::npos){
		inicio=str.find("CAF=", inicio)+4;
		delimitador=inicio;
		limite=str.find(";", inicio);
		while(delimitador<limite){ 
			delimitador=str.find("|", inicio);
	...
	inicio=str.find("CLNSIG=", inicio)+7;
	delimitador=inicio;
	limite=str.find(";", inicio);
	...
	size_t del2, del3, lim2=str.find(";", sdbid), lim3=str.find(";", bn);
	...
	while(delimitador<limite){
		...
		if(delimitador<limite){
			del2=str.find("|", sdbid);
			del3=str.find("|", bn);
			name=str.substr(bn, delimitador-bn);
			ID=str.substr(sdbid, del2-sdbid);
			database=str.substr(sdb, del3-sdb);
		}else{ // Sino hay hemos acabado
			name=str.substr(bn, limite-bn);
			ID=str.substr(sdbid, lim2-sdbid);
			database=str.substr(sdb, lim3-sdb);
		}
