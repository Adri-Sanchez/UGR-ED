...
int cuentaMutacionesEnfermedad(vector<mutacion> &  vm, const string & s) {
	...
	for (unsigned i=0; i<vm.size(); i++){    //recorrer vector de mutaciones vm
		...
		asociado=false;
		j=0;
		while(!asociado && j<enfermedades.size()){
			//utilizar el método enfermedades[j].nameContains(s)
			asociado=enfermedades[j++].nameContains(s);
		}
...
int main(int argc, char *argv[]){
	...
	load(vectorSNPs, "./data/clinvar_20160831.vcf");
